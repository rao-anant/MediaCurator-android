...
|---|---|---|---|---|
| Indexing resumable on OEM lock-kill | §7 | DONE a31 | DONE b18 | iOS was already resumable — both stores skip already-cached items on re-run; geo flushes every 50 and won't restart a running job. b18 batches the hash store to every 25 (was every item — O(n²) writes) + a final flush, matching Android's 20–50 |
