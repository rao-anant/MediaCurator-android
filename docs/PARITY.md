...
|---|---|---|---|---|
| Expand/collapse lands the row clear of the sticky header | §2, §3 | ? | DONE b23-b24 | iOS: opening a month scrolled its header to the true top, hiding the month's sub-header ("Camera & Others" / "WhatsApp") + first photo row BEHIND the floating sticky bar (user had to scroll up to see them); collapsing a month/year left the just-closed row scrolled off the top; collapsing a year left a stale sticky label + "Hide {month}" bar. iOS fix: measure the sticky bar height and land month-*open* just below it; land collapses and year-open at the true top; clear the open month when its year collapses. Android has sticky headers too — verify the scroll-to-expanded lands below the sticky decoration and that collapse keeps the acted-on row on screen |
...
| Stats "cleaned up (lifetime)" must not flash 0 while loading | §9 | ? | DONE b22 | iOS: the lifetime cleaned-up total is an instant prefs read but was published only AFTER the async media scan, so it flashed 0 then corrected. Now published before the scan. Android: confirm StatsDialog shows the persisted total immediately (likely already synchronous) |
