...
|---|---|---|---|---|
| Persistent (sticky) collapse control when scrolled deep | §2, §3 | TODO | DONE b25 | iOS pins a sticky year/month context bar with a collapse chevron, so you can close a deeply-expanded month/year WITHOUT scrolling back up to find its in-list tree chevron (which has scrolled off the top). The pinned chevron matches the tree (left-side `chevron.down`) and collapses ONE level at a time — month -> that year's month list -> all-years — like the geo drill (close Fremont -> California, not -> US). Android's in-list tree chevron is "lost" once you're deep in a month; add a pinned/sticky collapse affordance with the same one-level behavior. Requested by the user for Android |
| Duplicate hashing: photos-first, with a gated Duplicates entry point | §8 | ? | DONE b25 | iOS now hashes the library continuously in the background, PHOTOS FIRST (unlocks Duplicates) then videos, and disables the Home "Find duplicates" card with "Hashing photos N/M" until photos are done (videos fill in after). Serial + cancel-on-background = watchdog-safe. Android already hashes in the background via GalleryViewModel and shows a "N indexed" subtitle — verify the UX: photos-before-videos ordering + not letting the user open Duplicates on a partial photo set |
